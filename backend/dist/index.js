...
const { paymentWebhooksRouter, adminPaymentProvidersRouter, paymentsRouter, } = require('./routes/payment.routes');
const { adminIntegrationsRouter } = require('./routes/admin-integrations.routes');
const { walletRouter } = require('./routes/wallet.routes');
const { adminPricingRouter, identityLinkRouter, walletTransferRouter, tripRecordingRouter, } = require('./routes/phases-25-28.routes');
...
const { rideExperienceRouter, sosRouter, publicTripShareRouter, } = require('./routes/ride-experience.routes');
...
const { driverRouter, subscriptionsRouter, rentalsRouter, adminOpsRouter, adminFinanceRouter, adminRewardsRouter, inboxRouter, } = require('./routes/platform.routes');
...
const { rideBookingRouter, voiceRouter, channelWebhooksRouter, adminVehicleRouter, adminChannelsRouter, } = require('./routes/channels.routes');
...
app.post('/api/v1/auth/signup', async (req, res) => {
    ...
    try {
        const { email, phone, password, firstName, lastName, name, userType = 'customer', country: countryHint, city = 'Accra', } = req.body;
        ...
        const inserted = await authDb.query(`INSERT INTO users (email, phone, first_name, last_name, password, user_type, country, city, is_active, is_verified)
       ...
       RETURNING id, email, phone, first_name, last_name, user_type, country, city`, [
        ...
        if (dbUser.user_type === 'customer') {
            await authDb
                .query(`INSERT INTO customers (user_id, rating) VALUES ($1, 4.7)`, [dbUser.id])
                .catch(() => authDb.query(`INSERT INTO customers (user_id) VALUES ($1) ON CONFLICT DO NOTHING`, [
                dbUser.id,
            ]));
        }
        else if (dbUser.user_type === 'driver') {
            await authDb
                .query(`INSERT INTO drivers (user_id, vehicle_type, is_online, rating)
           VALUES ($1, 'standard', false, 5.0)`, [dbUser.id])
                .catch(() => undefined);
        }
        const token = jwt.sign({ id: dbUser.id, email: dbUser.email, userType: dbUser.user_type }, process.env.JWT_SECRET || 'secret', { expiresIn: '7d' });
        res.status(201).json({
            status: 'success',
...
app.post('/api/v1/auth/register', async (req, res) => {
    try {
        const { email, phone, password, firstName, lastName, name, userType = 'customer', country: countryHint, city = 'Accra', gender, dateOfBirth, date_of_birth, } = req.body;
        const cleanEmail = email ? String(email).trim().toLowerCase() : null;
        const cleanPhone = phone ? String(phone).replace(/[\s\-()]/g, '') : null;
        const fname = firstName || String(name || '').trim().split(/\s+/)[0] || null;
        const lname = lastName ||
        ...
        const cleanDob = dobRaw && /^\d{4}-\d{2}-\d{2}$/.test(String(dobRaw)) ? String(dobRaw) : null;
        if (!password) {
            return res.status(400).json({ status: 'error', message: 'Password is required' });
        }
        if (!cleanEmail && !cleanPhone) {
        ...
        const hash = await bcrypt.hash(password, 10);
        const inserted = await authDb.query(`INSERT INTO users (email, phone, first_name, last_name, password, user_type, country, city, gender, date_of_birth, is_active, is_verified)
...
app.post('/api/v1/auth/login', async (req, res) => {
    try {
        ...
        try {
            if (isEmail) {
                const r = await authDb.query(`SELECT id, email, phone, first_name, last_name, password, user_type, country, city, is_active
           FROM users WHERE lower(email) = lower($1) LIMIT 1`, [raw]);
                dbUser = r.rows[0];
            }
            else {
                const r = await authDb.query(`SELECT id, email, phone, first_name, last_name, password, user_type, country, city, is_active
        ...
        if (dbUser) {
            ...
            if (dbUser.user_type === 'admin') {
                try {
                    const rr = await authDb.query(`SELECT role FROM admin_roles WHERE user_id = $1`, [dbUser.id]);
                    roles = rr.rows.map((r) => r.role);
                }
                catch {
                    roles = [];
                }
            }
            const token = jwt.sign({ id: dbUser.id, email: dbUser.email, userType: dbUser.user_type, roles }, process.env.JWT_SECRET || 'secret', { expiresIn: '7d' });
        ...
        const user = Array.from(users.values()).find((u) => (isEmail && u.email === raw) ||
        ...
        const token = jwt.sign({ id: user.id, email: user.email, userType: user.userType }, process.env.JWT_SECRET || 'secret', { expiresIn: '7d' });
        res.status(200).json({
            status: 'success',
            data: {
                userId: user.id,
...
function otpLookupKeys(raw) {
    const normalized = normalizeAuthIdentifier(raw);
    const keys = new Set([String(raw || '').trim(), normalized].filter(Boolean));
    return [...keys];
}
...
async function persistOtp(opts) {
    ...
    if (opts.identifier !== storeKey)
        otpStore.set(opts.identifier, entry);
    try {
        await authDb.query(`UPDATE auth_otps SET consumed_at = NOW()
       WHERE identifier = $1 AND purpose = $2 AND consumed_at IS NULL`, [storeKey, opts.purpose]);
        await authDb.query(`INSERT INTO auth_otps (identifier, code_hash, purpose, user_id, expires_at)
       VALUES ($1, $2, $3, $4, to_timestamp($5 / 1000.0))`, [storeKey, hashOtp(opts.code), opts.purpose, opts.userId || null, expires]);
    }
    catch (e) {
...
async function findUserForPasswordReset(identifier) {
    ...
    if (!raw)
        return null;
    if (raw.includes('@')) {
        const r = await authDb.query(`SELECT id, email, phone FROM users WHERE lower(email) = lower($1) AND is_active = TRUE LIMIT 1`, [raw]);
        return r.rows[0] || null;
    }
    const phone = normalizeAuthIdentifier(raw);
    const r = await authDb.query(`SELECT id, email, phone FROM users
     WHERE phone = $1
        OR regexp_replace(COALESCE(phone, ''), '[^0-9+]', '', 'g') = $2
     LIMIT 1`, [raw, phone]);
    return r.rows[0] || null;
...
app.post('/api/v1/auth/send-code', async (req, res) => {
    try {
        ...
        const full = phone.startsWith('+') ? phone : `${countryCode.replace(/\s/g, '')}${phone.replace(/\D/g, '').replace(/^0/, '')}`;
...
app.get('/api/v1/notifications', authenticateToken, async (req, res) => {
    try {
        ...
        let rows = await authDb
            .query(`SELECT id, title, body, is_read, category, icon_key, metadata, created_at
        ...
        if (!rows.rows.length) {
            ...
            const inbox = await authDb
                .query(`SELECT id, title, body, read AS is_read, category::text AS category, created_at
           FROM inbox_messages WHERE user_id = $1
           ORDER BY created_at DESC LIMIT 50`, [uid])
                .catch(() => ({ rows: [] }));
            rows = {
                rows: inbox.rows.map((m) => ({
                    ...m,
                    icon_key: String(m.category || '').includes('ride')
                        ? 'ride'
...
app.patch('/api/v1/notifications/mark-all-read', authenticateToken, async (req, res) => {
    try {
        await authDb
...
app.get('/api/v1/users/me/profile', authenticateToken, async (req, res) => {
    try {
        const uid = req.user.id;
        const user = await authDb.query(`SELECT id, email, phone, first_name, last_name, avatar_url, country, gender, onboarding_step, phone_verified_at
       FROM users WHERE id = $1`, [uid]);
        const u = user.rows[0] || {};
        const rides = await authDb
            .query(`SELECT COUNT(*)::int AS c, COALESCE(AVG(rating),0)::float AS rating
         FROM rides WHERE customer_id = $1 AND status = 'completed'`, [uid])
            .catch(() => ({ rows: [{ c: 0, rating: 0 }] }));
        const points = await authDb
            .query(`SELECT COALESCE(points_balance, balance_points, 0)::float AS points
         FROM wallets WHERE user_id = $1`, [uid])
            .catch(() => ({ rows: [{ points: 0 }] }));
        const unread = await authDb
            .query(`SELECT COUNT(*)::int AS c FROM user_notifications WHERE user_id = $1 AND is_read = false`, [uid])
            .catch(() => ({ rows: [{ c: 0 }] }));
...
app.patch('/api/v1/users/me/profile-setup', authenticateToken, async (req, res) => {
    try {
        const uid = req.user.id;
        ...
        await authDb.query(`UPDATE users SET
         first_name = $2,
         last_name = $3,
         ...
         updated_at = NOW()
       WHERE id = $1`, [uid, firstName, lastName, email === undefined ? null : email, gender || null, avatarUrl || null, step]).catch(async () => {
            await authDb.query(`UPDATE users SET first_name = $2, last_name = $3, updated_at = NOW() WHERE id = $1`, [uid, firstName, lastName]);
        });
        res.json({
            status: 'success',
            data: {
...
app.get('/api/v1/rides/:id/receipt', authenticateToken, async (req, res) => {
    try {
        ...
        const shape = (raw) => {
            ...
            return {
                ...
                distanceLabel: `${Number(raw.distance_km ?? raw.distanceKm ?? 8.4)} km · ${Number(raw.duration_minutes ?? raw.durationMinutes ?? 18)} min`,
        ...
        const byRef = await authDb
            .query(`SELECT r.*, rr.txn_ref, rr.service_label AS rr_service, rr.driver_name, rr.pickup_label,
        ...
        await authDb
            .query(`INSERT INTO ride_receipts (
...
app.post('/api/v1/auth/resend-otp', async (req, res) => {
    try {
        ...
        await persistOtp({ identifier, code, purpose: purpose, userId });
        logger.info(`Resend OTP (${purpose}) for ${identifier}: ${code}`);
        const data = { identifier: storeKey, phone: storeKey, expiresInSeconds: 600 };
        if (process.env.NODE_ENV !== 'production' || process.env.EXPOSE_OTP === 'true') {
            data.devCode = code;
        }
...
app.post('/api/v1/auth/verify-otp', async (req, res) => {
    try {
        ...
        if (!entry) {
            try {
                const storeKey = normalizeAuthIdentifier(identifier);
                const dbOtp = await authDb.query(`SELECT * FROM auth_otps
           WHERE identifier = ANY($1::text[])
             AND purpose = $2
             AND consumed_at IS NULL
             AND expires_at > NOW()
           ORDER BY created_at DESC
           LIMIT 1`, [otpLookupKeys(identifier), purpose === 'reset' ? 'reset' : 'signup']);
                ...
                if (row && row.code_hash === hashOtp(code)) {
                    ...
                    await authDb.query(`UPDATE auth_otps SET attempts = attempts + 1 WHERE id = $1`, [row.id]);
                }
            }
            catch (e) {
        ...
        try {
            await authDb.query(`UPDATE auth_otps SET consumed_at = NOW()
         WHERE identifier = ANY($1::text[]) AND purpose = $2 AND consumed_at IS NULL`, [otpLookupKeys(identifier), entry.purpose]);
        }
        catch {
            /* optional */
        }
        if (purpose !== 'reset' && entry.purpose !== 'reset' && !identifier.includes('@')) {
            await authDb
                .query(`UPDATE users SET phone_verified_at = COALESCE(phone_verified_at, NOW()),
             onboarding_step = GREATEST(COALESCE(onboarding_step, 1), 1),
             updated_at = NOW()
           WHERE phone = ANY($1::text[]) OR regexp_replace(COALESCE(phone,''), '\\D', '', 'g')
             = regexp_replace($2, '\\D', '', 'g')`, [otpLookupKeys(identifier), identifier])
                .catch(() => undefined);
        }
        if (purpose === 'reset' || entry.purpose === 'reset') {
            ...
            const resetToken = jwt.sign({ id: entry.userId, purpose: 'password_reset' }, process.env.JWT_SECRET || 'secret', { expiresIn: '15m' });
            return res.json({
                status: 'success',
...
app.post('/api/v1/auth/reset-password', async (req, res) => {
    try {
        ...
        const updated = await authDb.query(`UPDATE users SET password = $1 WHERE id = $2 AND COALESCE(is_active, TRUE) = TRUE
       RETURNING id, email, phone`, [hash, payload.id]);
        if (!updated.rows[0]) {
...
app.get('/api/v1/rides/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)) {
            ...
            const byRef = await authDb.query(`SELECT id FROM rides WHERE public_ref = $1 LIMIT 1`, [id]).catch(() => ({ rows: [] }));
            if (byRef.rows[0]?.id) {
        ...
        const disputed = String(row.dispute_status || '').toLowerCase() === 'disputed' ||
        ...
        const timeline = [
            ...
            {
                key: 'en_route',
                label: status.includes('arrived') || status.includes('progress') || status === 'completed'
                    ? 'Driver en route'
...
app.get('/api/v1/rides', authenticateToken, async (req, res) => {
    try {
        ...
        let dbRides = [];
        try {
            const result = await authDb.query(`SELECT id, pickup_address, dropoff_address, status,
        ...
        const memRides = Array.from(rides.values()).filter((r) => r.customerId === customerId);
        res.status(200).json({
            status: 'success',
            data: {
...
app.post('/api/v1/wallet/topup', authenticateToken, async (req, res) => {
    try {
        ...
        const wallet = await authDb.query(`SELECT id FROM wallets WHERE user_id = $1 ORDER BY last_updated ASC NULLS LAST LIMIT 1`, [userId]);
        let walletId = wallet.rows[0]?.id;
        if (!walletId) {
            const created = await authDb.query(`INSERT INTO wallets (user_id, balance_fiat, currency)
         VALUES ($1, 0, $2) RETURNING id`, [userId, currency]);
            walletId = created.rows[0].id;
        }
        await authDb.query(`UPDATE wallets SET balance_fiat = COALESCE(balance_fiat, 0) + $1, last_updated = NOW() WHERE id = $2`, [amount, walletId]);
        await authDb
            .query(`INSERT INTO wallet_transactions_v2 (wallet_id, type, amount, reference)
         VALUES ($1, 'topup', $2, $3)`, [walletId, amount, reference])
            .catch(() => undefined);
...
app.get('/api/v1/users/notification-prefs', authenticateToken, async (req, res) => {
    const defaults = {
...
app.patch('/api/v1/users/notification-prefs', authenticateToken, async (req, res) => {
    const uid = req.user?.id || 'anon';
    ...
    try {
        if (typeof req.body.notifications_enabled === 'boolean') {
            await authDb.query(`INSERT INTO user_settings (user_id, notifications_enabled)
         VALUES ($1, $2)
         ON CONFLICT (user_id) DO UPDATE
         SET notifications_enabled = EXCLUDED.notifications_enabled, updated_at = NOW()`, [uid, req.body.notifications_enabled]);
        }
    }
    catch (e) {
...
app.get('/api/v1/users/me', authenticateToken, async (req, res) => {
    try {
        const uid = req.user?.id;
        const result = await authDb.query(`SELECT u.id, u.email, u.phone, u.first_name, u.last_name, u.avatar_url,
...
app.put('/api/v1/users/profile', authenticateToken, async (req, res) => {
    try {
        ...
        const country = req.body.country != null ? String(req.body.country).trim().toUpperCase() : undefined;
        ...
        const dateOfBirth = dobRaw == null
            ...
            : /^\d{4}-\d{2}-\d{2}$/.test(String(dobRaw))
                ? String(dobRaw)
                : null;
        const result = await authDb.query(`UPDATE users SET
         ...
         updated_at = NOW()
       WHERE id = $1
       RETURNING id, email, phone, first_name, last_name, user_type, country, city, gender, date_of_birth, avatar_url, is_verified`, [uid, firstName ?? null, lastName ?? null, phone ?? null, country ?? null, city ?? null, gender ?? null, dateOfBirth ?? null]);
        const u = result.rows[0];
        if (!u) {
...
app.patch('/api/v1/users/me/settings', authenticateToken, async (req, res) => {
    try {
        const uid = req.user.id;
        const { notificationsEnabled, language = 'English', region = 'Ghana', } = req.body;
        const enabled = typeof notificationsEnabled === 'boolean' ? notificationsEnabled : true;
        const result = await authDb.query(`INSERT INTO user_settings (user_id, notifications_enabled, language, region)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING *`, [uid, enabled, language, region]);
...
// ============================================
app.get('/api/v1/public/app-links', async (_req, res) => {
    try {
        const result = await authDb.query(`SELECT ios_url, android_url, updated_at FROM app_store_links WHERE id = 1 LIMIT 1`);
        const row = result.rows?.[0];
        res.json({
            status: 'success',
            data: {
                ios_url: row?.ios_url || 'https://apps.apple.com/app/movr',
                android_url: row?.android_url || 'https://play.google.com/store/apps/details?id=io.movr.app',
...
app.get('/api/v1/public/locales', async (_req, res) => {
    const fallback = [
        {
            country_code: 'GH',
            country_name: 'Ghana',
    ...
    try {
        const rows = await authDb.query(`SELECT country_code, country_name, language_code, language_label,
       ...
       ORDER BY sort_order ASC, country_name ASC`);
        ...
        const countries = await authDb.query(`SELECT code AS country_code, name AS country_name
       FROM countries
       WHERE is_active = TRUE
       ORDER BY name ASC`);
...
// ============================================
app.get('/api/v1/public/help/categories', async (req, res) => {
    try {
        const q = String(req.query.q || '').trim().toLowerCase();
        const cats = await authDb.query(`SELECT id, slug, title, description, icon_key, sort_order
       FROM help_categories
       WHERE is_active = TRUE
       ORDER BY sort_order ASC, title ASC`);
        let rows = cats.rows;
        if (q) {
            const arts = await authDb.query(`SELECT c.slug AS category_slug
         ...
         WHERE a.is_active = TRUE
           AND (
            ...
            const match = new Set(arts.rows.map((r) => r.category_slug));
            rows = rows.filter((c) => match.has(c.slug) ||
...
app.get('/api/v1/public/help/categories/:slug', async (req, res) => {
    try {
        const cat = await authDb.query(`SELECT * FROM help_categories WHERE slug = $1 AND is_active = TRUE LIMIT 1`, [req.params.slug]);
        if (!cat.rows[0]) {
            return res.status(404).json({ status: 'error', message: 'Category not found' });
        }
        const articles = await authDb.query(`SELECT id, slug, title, body, keywords, sort_order
       FROM help_articles
       WHERE category_id = $1 AND is_active = TRUE
       ORDER BY sort_order ASC`, [cat.rows[0].id]);
        res.json({
            status: 'success',
...
app.get('/api/v1/public/legal/:slug', async (req, res) => {
    try {
        const doc = await authDb.query(`SELECT id, slug, title, updated_label, updated_at
       ...
       LIMIT 1`, [req.params.slug]);
        ...
        const sections = await authDb.query(`SELECT section_number, title, body
       FROM legal_sections
       WHERE document_id = $1
       ORDER BY sort_order ASC, section_number ASC`, [doc.rows[0].id]);
        res.json({
            status: 'success',
...
app.get('/api/v1/public/status-copy/:key', async (req, res) => {
    const defaults = {
        no_connection: {
            title: 'No connection',
    ...
    try {
        const row = await authDb.query(`SELECT key, title, body, cta_label, meta FROM app_status_copy WHERE key = $1 LIMIT 1`, [req.params.key]).catch(() => authDb.query(`SELECT key, title, body, cta_label FROM app_status_copy WHERE key = $1 LIMIT 1`, [
            req.params.key,
        ]));
        res.json({
            status: 'success',
...
app.get('/api/v1/public/offline-capabilities', async (_req, res) => {
    try {
        const rows = await authDb.query(`SELECT id, label, icon_key, sort_order FROM offline_capability_catalog
...
app.get('/api/v1/public/onboarding', async (_req, res) => {
    const fallback = [
        {
            sort_order: 1,
    ...
    try {
        const [rows, landing] = await Promise.all([
            authDb.query(`SELECT sort_order, title, body, icon_key
...
app.get('/api/v1/me/home-dashboard', authenticateToken, async (req, res) => {
    try {
        const uid = req.user.id;
        const user = await authDb.query(`SELECT id, first_name, last_name, avatar_url, home_address, home_lat, home_lng, city, country
       FROM users WHERE id = $1`, [uid]);
        const u = user.rows[0] || {};
        const wallet = await authDb
            .query(`SELECT COALESCE(balance_fiat,0)::float AS balance,
        ...
        const tokens = await authDb
            .query(`SELECT COALESCE(balance_pending,0)+COALESCE(balance_onchain,0)::float AS tokens
         FROM token_balances WHERE user_id = $1`, [uid])
            .catch(() => ({ rows: [] }));
        const rides = await authDb
            .query(`SELECT id, 'ride' AS kind, COALESCE(dropoff_address, 'Ride') AS title,
         ...
         ORDER BY COALESCE(completed_at, created_at) DESC LIMIT 5`, [uid])
            .catch(() => ({ rows: [] }));
        const deliveries = await authDb
            .query(`SELECT id, 'deliver' AS kind, 'Package Delivery' AS title,
...
app.get('/api/v1/wallet/payment-methods', authenticateToken, async (req, res) => {
    try {
        const wantCatalog = String(req.query.catalog || '') === '1' ||
            String(req.query.purpose || '') === 'topup';
        if (wantCatalog) {
            const catalog = await authDb
                .query(`SELECT id, label, subtitle, icon_key, sort_order
             FROM wallet_topup_methods WHERE is_active = TRUE ORDER BY sort_order`)
                .catch(() => ({ rows: [] }));
            if (catalog.rows.length) {
                return res.json({
                    status: 'success',
                    data: catalog.rows.map((m, i) => ({
                        id: m.id,
                        provider: m.label,
                        method_type: m.id,
                        label: m.label,
        ...
        const rows = await authDb.query(`SELECT id, provider, method_type, label, last_four, is_default
         FROM customer_payment_methods
         WHERE user_id = $1
         ORDER BY is_default DESC, created_at ASC`, [req.user.id]);
        if (rows.rows.length) {
            return res.json({ status: 'success', data: rows.rows });
        }
        res.json({
...
io.on('connection', (socket) => {
    ...
    socket.on('ride-chat:join', (rideId) => {
        if (rideId)
            socket.join(`ride-chat:${rideId}`);
    });
    socket.on('ride-chat:message', async (data) => {
        if (!data?.rideId || !data?.body)
            return;
        try {
            const { DatabaseService } = require('./services/database.service');
            const chatDb = new DatabaseService();
            if (data.senderId) {
                await chatDb.query(`INSERT INTO ride_messages (ride_id, sender_id, body) VALUES ($1, $2, $3)`, [data.rideId, data.senderId, String(data.body).slice(0, 2000)]);
            }
        }
        catch (err) {
    ...
    socket.on('rental:join', (rentalId) => {
        if (rentalId)
            socket.join(`rental:${rentalId}`);
    });
    socket.on('rental:location', (data) => {
        if (data?.rentalId) {
